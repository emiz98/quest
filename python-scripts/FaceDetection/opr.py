...
def remap(value_to_map, new_range_min, new_range_max, old_range_min, old_range_max):
    ...
    remapped_val = (value_to_map - old_range_min) * (new_range_max -
                                                     new_range_min) / (old_range_max - old_range_min) + new_range_min
...
def find_face(image_to_check, max_target_distance):
    ...
    gray = cv2.cvtColor(image_to_check, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, 1.1, 20)  # look for faces
    ...
    if len(faces) >= 1:  # if face(s) detected
        ...
        # why? can't remember why I did this
        distance_from_center_X = (center_face_X - width/2)/220
        distance_from_center_Y = (center_face_Y - height/2)/195  # why?

        # calculate distance between image center and face center
        target_distance = math.sqrt(
            (distance_from_center_X*220)**2 + (distance_from_center_Y*195)**2)
        ...
        cv2.circle(image_to_check, (int(width/2), int(height/2)),
                   int(max_target_distance), color, 2)  # draw circle
